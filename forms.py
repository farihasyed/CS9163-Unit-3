...
class Registration(FlaskForm):
    ...
    uname = StringField('Username: ', validators=USERNAME_VALIDATORS)
    pword = PasswordField('Password: ', validators=registration_password_validators)
...
class Login(FlaskForm):
    ...
    uname = StringField('Username: ', validators=USERNAME_VALIDATORS)
    pword = PasswordField('Password: ', validators=login_password_validators)
    phone = IntegerField('Phone (XXXXXXXXXX): ', validators=PHONE_VALIDATORS)
    sign_in = SubmitField()


class SpellCheck(FlaskForm):
    inputtext = TextAreaField('Input', validators=INPUT_VALIDATORS)
    submit = SubmitField()
    logout = SubmitField()
